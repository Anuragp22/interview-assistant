...
| **Session status** | `awaiting-cv → awaiting-call → in-call → awaiting-report → completed`, plus `abandoned` (`types/index.d.ts`). The agent additionally accepts `reconnecting` as a callable state (`session_data.py`); the browser tracks its own `reconnecting` connection state (`SessionRoomClient.tsx`). |
