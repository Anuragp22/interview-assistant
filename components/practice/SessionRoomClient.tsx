...
export default function SessionRoomClient({
  ...
  async function startCall() {
    ...
    room.on(RoomEvent.Disconnected, (_reason?: DisconnectReason) => {
      ...
      // the report gets generated whether or not this browser is still alive
      // to say so. The report page waits for it to land.
  ...
  async function endCall() {
    ...
    if (room) {
      // Disconnecting ends the agent's job, which runs its teardown and hands
      // the session off to scoring. We don't wait for any of that here.
