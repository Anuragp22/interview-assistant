...
| **Voice pipeline** | LiveKit Cloud (WebRTC SFU) + a **Python agent worker** under `livekit-agent/` that wires Deepgram STT → OpenAI GPT-4 → 11labs TTS |
...
livekit-agent/                        ── Separate Python service ──────────────────────────────────────
  ...
  src/interview_agent/
    ...
    pipeline.py                       Wires Deepgram STT → GPT-4 LLM → 11labs TTS
...
5. Inside the agent (`pipeline.py`): Deepgram Nova-2 transcribes user audio → GPT-4 (system prompt built by `prompts.build_system_prompt`) generates a reply → 11labs synthesises with `voice_settings()` → audio is sent back through LiveKit.
